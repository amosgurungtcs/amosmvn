...
public class FormsListServlet extends AbstractPredicateServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            // setCharacterEncoding before JSONWriter setup, otherwise ISO-8859-1 gets used. Reference CQ-35521.
            resp.setCharacterEncoding("utf-8");
            JSONWriter w = new JSONWriter(resp.getWriter());
            FormsManager formsManager = req.getResourceResolver().adaptTo(FormsManager.class);
            if ( req.getRequestURI().contains("/actions") ) {
                resp.setContentType("application/json");
                resp.setCharacterEncoding("utf-8");

                writeActions(w, formsManager, i18n);
            } else if ( req.getRequestURI().contains("/constraints") ) {
                resp.setContentType("application/json");
                resp.setCharacterEncoding("utf-8");

                writeConstraints(w, formsManager, i18n);
            } else if ( req.getRequestURI().contains("/actiondialog") ) {
                final String dialogPath = formsManager.getDialogPathForAction(req.getParameter("id"));
                if ( dialogPath != null ) {
    ...
    private void writeJson(Iterator<ComponentDescription> descIter, JSONWriter w, boolean writeEmpty, I18n i18n)
    throws JSONException {
        ...
        while (descIter.hasNext() ) {
            ...
            w.key("text");
            w.value(i18n.get(desc.getTitle()));
     ...
     */
    private void writeActions(JSONWriter w, FormsManager formsManager, I18n i18n)
    throws JSONException {
        writeJson(formsManager.getActions(), w, false, i18n);
    }

    /**
     * Generates a JSON export of constraints
     */
    private void writeConstraints(JSONWriter w, FormsManager formsManager, I18n i18n)
    throws JSONException {
        writeJson(formsManager.getConstraints(), w, true ,i18n);
   }

    /**
     * Generates an empty dialog JSON export
     */
    private void writeEmptyDialog(JSONWriter w) throws JSONException {
        w.object();
        w.key("jcr:primaryType").value("cq:WidgetCollection");
        w.endObject();
    }
