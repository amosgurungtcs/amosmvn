...
public class ColumnCtlPostProcessor implements SlingPostProcessor {
    ...
    private final static String COLCTL_POSTPROCESSOR_SERVICE="colctl-postprocessor-service";
    ...
    protected void activate(ComponentContext context) {
        try {
            serviceSession = repository.loginService(COLCTL_POSTPROCESSOR_SERVICE,null);
        } catch (RepositoryException e) {
            log.error("unable to initialize service session.");
        }
    }
    ...
    public void process(SlingHttpServletRequest request, List<Modification> changes)
        ...
        for (Modification mod : changes) {
            switch (mod.getType()) {
                case DELETE:
                    // check for deleted paras using the service session because
                    // the nodes are already deleted at this stage
                    String colctrlProperty = null;
                    try {
                        if (serviceSession.nodeExists(mod.getSource())) {
                            colctrlProperty = mod.getSource() + "/sling:resourceType";
                            String rt = serviceSession.getProperty(colctrlProperty).getString();
