...
public class FormsHelper {
    ...
    /**
     * Push a temporary read-only state.  (See {@link #setFormReadOnly(SlingHttpServletRequest)}
     * for further details of read-only states.)
     *
     * @param request the current request
     * @return a token for use by {@link #popFormReadOnly(SlingHttpServletRequest, Object)}
     */
    public static Object pushFormReadOnly(final SlingHttpServletRequest request) {
        boolean previousState = isReadOnly(request);
        request.setAttribute(REQ_ATTR_READ_ONLY, true);
     ...
     * @param request the current request
     * @param previousState The token returned by {@link #pushFormReadOnly(SlingHttpServletRequest)}
     */
    public static void popFormReadOnly(final SlingHttpServletRequest request, Object previousState) {
        if (!((Boolean) previousState)) {
            request.removeAttribute(REQ_ATTR_READ_ONLY);
        }
    }
    ...
    /**
     * Indicates whether posted form values should be passed through an expression evaluator. It will be up to
     * each component to decide how it react to this attribute.
     * @param req The current request.
     * @return <code>true</code> if expressions should be evaluated on posted values
     */
    public static boolean allowExpressions(final SlingHttpServletRequest req) {
        Boolean expressionsEnabledAttr = (Boolean) req.getAttribute(REQ_ATTR_EXPRESSIONS_ENABLED);
        if (expressionsEnabledAttr == null) {
            return true;
        } else {
            return expressionsEnabledAttr.booleanValue();
        }
    }
