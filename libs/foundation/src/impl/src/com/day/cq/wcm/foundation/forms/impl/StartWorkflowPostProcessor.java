...
@Component
@Service
@Property(name="name", value="StartWorkflowPostProcessor")
public class StartWorkflowPostProcessor implements SlingPostProcessor {
    
    private final Logger log = LoggerFactory.getLogger(getClass());

    private final static String START_WORKFLOW_SERVICE = "start-workflow-service";
    ...
    public void process(SlingHttpServletRequest request, List<Modification> mods) throws Exception {
        ...
        if(workflowModelPath == null) {
            ...
            // Start workflow on supplied content, using service user session
            log.info("Starting workflow {} on path {}", workflowModelPath, payloadPath);
            final Session serviceUserSession = repository.loginService(START_WORKFLOW_SERVICE, null);
            try {
                final WorkflowSession wfs = workflowService.getWorkflowSession(serviceUserSession);
