...
public final class Search {
    ...
    public final class Result {
        ...
        public String getTrackerScript() {
            ...
            if (!search.getQuery().startsWith(SimpleSearch.RELATED_PREFIX)) {
                ...
                sb.append("{q:'").append(encodeURL(search.getQuery())).append("',");
                sb.append("nr:'").append(getTotalMatches()).append("',");
                sb.append("et:'").append(getExecutionTimeMillis()).append("',");
            ...
            sb.append("q:'").append(encodeURL(search.getQuery().toLowerCase())).append("',\n");
    ...
    /*
     * Adds node types predicate to the search:
     * Takes the node types from the request parameters if defined. Otherwise it sets the default node types
     */
    private void addNodeTypesPredicate() {
        ...
        if (nodeTypeParams != null && nodeTypeParams.length > 0) {
            ...
            addNodeTypesPredicate(nodeTypes);
        } else {
            addNodeTypesPredicate(defaultNodeTypes);
        }
    }
